...
import com.yz.yikfrl.yizhilearning.model.bean.gankio.GankIoDayBean;
...
public class GankIoDayAdapter extends BaseMultiItemQuickAdapter<GankIoDayItemBean, BaseViewHolder>{
    ...
    public GankIoDayAdapter(@NonNull List<GankIoDayItemBean> data){
        super(data);
        mlist = data;

        addItemType(GankIoDayItemBean.GANK_IO_DAY_ITEM_DAY_NORMAL, R.layout.item_gank_io_day_normal);
        addItemType(GankIoDayItemBean.GANK_IO_DAY_ITEM_DAY_REFRESH, R.layout.item_gank_io_day_refresh);
    }
     ...
     * @param bean     bean
     */
    public void refreshItem(int position, GankIoDayItemBean bean){
    ...
    protected void convert(BaseViewHolder helper, GankIoDayItemBean item) {
        ...
        switch (item.getType()){
            ...
            case "iOS":
                ...
                helper.setImageResource(R.id.iv_item_title, mResAndroid[mIOSIndex % 3]);
                break;
            case "前端":
        ...
        switch (helper.getItemViewType()){
            case GankIoDayItemBean.GANK_IO_DAY_ITEM_DAY_NORMAL:
                ...
                break;
            case GankIoDayItemBean.GANK_IO_DAY_ITEM_DAY_REFRESH:
                helper.addOnClickListener(R.id.ll_more).addOnClickListener(R.id.ll_refresh);
                break;
            default:
                break;
        }
